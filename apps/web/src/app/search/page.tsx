...
function SearchPageContent() {
  ...
  const categories = [
    { value: BusinessCategory.HOTEL, label: 'Hotels', color: 'bg-purple-100 text-purple-600' },
    { value: BusinessCategory.TOUR, label: 'Tours', color: 'bg-orange-100 text-orange-600' },
    { value: BusinessCategory.TRANSPORT, label: 'Transport', color: 'bg-blue-100 text-blue-600' },
        ...
        <form onSubmit={handleSearch} className="mb-8">
          <div className="relative max-w-2xl mx-auto">
            ...
            <input
              ...
              className="w-full pl-12 pr-32 py-4 text-lg border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
            <button
              type="submit"
              className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 font-medium"
            >
              Search
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            ...
            {categories.map((category) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                  selectedCategory === category.value ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-300'
                }`}
              >
                {category.label}
              </button>
                ...
                <div key={business.id} className="bg-white rounded-lg shadow-md p-6">
                  <div className="mb-4">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      {business.verified && (
                        <span className="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded-full ml-2">✓ Verified</span>
                      )}
                    </div>
                    <span className={`${getCategoryBadgeClass(business.category as BusinessCategory)} text-xs font-medium px-2.5 py-0.5 rounded-full inline-block`}>
                      {business.category}
                    </span>
                  </div>
                    ...
                    {business.whatsapp && (
                      <div className="flex items-center justify-between">
                        ...
                        <a
                          ...
                          className="bg-green-600 text-white text-sm py-1 px-3 rounded-md hover:bg-green-700"
                        >
                          WhatsApp
                        </a>
                      </div>
